...
<head>
  ...
  <title>Historial de {{ $user->username }}</title>
...
<body>
  <div class="top">
    <div><strong>Historial</strong> · {{ $user->name }} ({{ $user->username }})</div>
    <div>
      <a class="btn" href="{{ route('admin.pending') }}">Pendientes</a>
      <a class="btn" href="{{ route('admin.history.all') }}" style="margin-left:6px;">Historial general</a>
    </div>
  </div>

  <div class="wrap">
    <form method="GET" action="{{ route('admin.history', $user) }}" style="display:flex; gap:12px; align-items:flex-end; margin-bottom:12px; flex-wrap:wrap;">
      <div>
        <label style="display:block; font-weight:600; font-size:12px; color:#475569;">Desde</label>
        <input type="date" name="from" value="{{ request('from') }}" style="padding:6px 8px; border:1px solid #e5e7eb; border-radius:8px;">
      </div>
      <div>
        <label style="display:block; font-weight:600; font-size:12px; color:#475569;">Hasta</label>
        <input type="date" name="to" value="{{ request('to') }}" style="padding:6px 8px; border:1px solid #e5e7eb; border-radius:8px;">
      </div>
      <div>
        <label style="display:block; font-weight:600; font-size:12px; color:#475569;">Buscar</label>
        <input type="text" name="q" value="{{ request('q') }}" placeholder="Texto, IP, navegador..." style="padding:6px 8px; border:1px solid #e5e7eb; border-radius:8px; min-width:240px;">
      </div>
      <div>
        <label style="display:block; font-weight:600; font-size:12px; color:#475569;">Acción</label>
        <select name="action" style="padding:6px 8px; border:1px solid #e5e7eb; border-radius:8px;">
      ...
      <div>
        ...
        <a class="btn" href="{{ route('admin.history', $user) }}" style="margin-left:6px;">Limpiar</a>
        <a class="btn" href="{{ route('admin.history.export', array_merge(['user'=>$user->id], request()->query())) }}" style="margin-left:6px;">Exportar CSV</a>
      </div>
    </form>
    @if($logs->isEmpty())
